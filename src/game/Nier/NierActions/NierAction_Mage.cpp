...
void NierAction_Mage::InitializeCharacter(uint32 pmTargetLevel, uint32 pmSpecialtyTabIndex)
{
	if (!me)
	{
		return;
...
void NierAction_Mage::ResetTalent()
{
	if (!me)
	{
		return;
...
bool NierAction_Mage::InitializeEquipments(bool pmReset)
{
	if (!me)
	{
		return true;
...
bool NierAction_Mage::DPS(Unit* pmTarget, bool pmRushing, float pmDistanceMax, float pmDistanceMin, bool pmHolding, bool pmInstantOnly, bool pmChasing)
{
	if (!me)
	{
		return false;
...
bool NierAction_Mage::AOE(Unit* pmTarget, bool pmRushing, float pmDistanceMax, float pmDistanceMin, bool pmHolding, bool pmInstantOnly, bool pmChasing)
{
	if (!me)
	{
		return false;
...
bool NierAction_Mage::Cure(Unit* pmTarget)
{
	if (!me)
	{
		return false;
...
bool NierAction_Mage::Buff(Unit* pmTarget)
{
	if (!me)
	{
		return false;
...
bool NierAction_Mage::Mark(Unit* pmTarget, int pmRTI)
{
	if (pmRTI >= 0)
	{
...
bool NierAction_Mage::Assist(int pmRTI)
{
	if (pmRTI >= 0)
	{
		if (spell_Polymorph > 0)
...
uint32 NierAction_Mage::Caution()
{
	if (me)
	{
		if (me->IsAlive())
