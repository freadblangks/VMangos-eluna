...
NierAction_Druid::NierAction_Druid(Player* pmMe) :NierAction_Base(pmMe)
{
	spell_InsectSwarm = 0;
	spell_MoonkinForm = 0;
	spell_Typhoon = 0;
...
void NierAction_Druid::InitializeCharacter(uint32 pmTargetLevel, uint32 pmSpecialtyTabIndex)
{
	if (!me)
	{
		return;
...
void NierAction_Druid::ResetTalent()
{
	if (!me)
	{
		return;
...
bool NierAction_Druid::InitializeEquipments(bool pmReset)
{
	if (!me)
	{
		return true;
...
bool NierAction_Druid::DPS(Unit* pmTarget, bool pmRushing, float pmDistanceMax, float pmDistanceMin, bool pmHolding, bool pmInstantOnly, bool pmChasing)
{
	if (!me)
	{
		return false;
...
bool NierAction_Druid::AOE(Unit* pmTarget, bool pmRushing, float pmDistanceMax, float pmDistanceMin, bool pmHolding, bool pmInstantOnly, bool pmChasing)
{
	if (!me)
	{
		return false;
...
bool NierAction_Druid::Cure(Unit* pmTarget)
{
	if (!me)
	{
		return false;
...
bool NierAction_Druid::Buff(Unit* pmTarget)
{
	if (!me)
	{
		return false;
...
bool NierAction_Druid::Revive(Player* pmTarget)
{
	if (!me)
	{
		return false;
...
bool NierAction_Druid::Mark(Unit* pmTarget, int pmRTI)
{
	if (pmRTI >= 0)
	{
...
bool NierAction_Druid::Assist(int pmRTI)
{
	if (pmRTI >= 0)
	{
		if (spell_Hibernate > 0)
