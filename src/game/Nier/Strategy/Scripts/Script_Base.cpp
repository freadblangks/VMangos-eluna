...
bool NierMovement::Chase(Unit* pmChaseTarget, float pmChaseDistanceMin, float pmChaseDistanceMax, uint32 pmLimitDelay)
{
	if (!me)
	{
		return false;
	...
	if (activeMovementType == NierMovementType::NierMovementType_Chase)
	{
		if (chaseTarget)
		{
...
void NierMovement::MovePosition(Position pmTargetPosition, uint32 pmLimitDelay)
{
	MovePosition(pmTargetPosition.x, pmTargetPosition.y, pmTargetPosition.z, pmLimitDelay);
}

void NierMovement::MovePosition(float pmX, float pmY, float pmZ, uint32 pmLimitDelay)
{
	if (!me)
	{
		return;
	...
	if (activeMovementType == NierMovementType::NierMovementType_Point)
	{
...
void NierMovement::MovePoint(float pmX, float pmY, float pmZ)
{
	if (me)
	{
		if (me->GetStandState() != UnitStandStateType::UNIT_STAND_STATE_STAND)
...
void NierMovement::Update(uint32 pmDiff)
{
	if (!me)
	{
		return;
...
bool Script_Base::CastSpell(Unit* pmTarget, std::string pmSpellName, bool pmCheckAura, bool pmOnlyMyAura, bool pmClearShapeShift)
{
	...
	if (pmTarget)
	{
		...
		if (pmCheckAura)
		{
			if (pmOnlyMyAura)
			{
				if (sNierManager->HasAura(pmTarget, pmSpellName, me))
				{
					return false;
				}
			}
			...
			{
				if (sNierManager->HasAura(pmTarget, pmSpellName))
				{
					return false;
				}
			}
...
bool Script_Base::CancelAura(std::string pmSpellName)
{
	...
	std::set<uint32> spellIDSet = sNierManager->spellNameEntryMap[pmSpellName];
